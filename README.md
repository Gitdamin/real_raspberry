...
- 이후 카메라를 통해 사람이 존재하는지 확인
  - openCV의 HaarCascade 방식을 이용해 full + upper body 인식 
- 일정 시간 이상 외부인이 인식될 경우 비디오 녹화 시작
- 실시간 영상 스트리밍 기능 ON + 인터넷 로드 시작 
...
#### :zap: flask 서버 – 실시간 영상 스트리밍
- 파이썬으로 작성된 웹 프레임워크
- 페이지 정보에 대한 html 문서 작성 후 templates 폴더에 위치  
- 카메라 영상의 frame을 전달받는 함수 작성
- 웹 브라우저에서 [라즈베리파이 IP주소] : [설정된 포트번호] 입력 후 영상 스트리밍 확인 가능
_____________
## :star: 특장점

* 외부인이 벨을 누르고 얼굴을 카메라 앞에 항상 대기하고 있지 않기 때문에 발생하는 신원파악 문제를 쉽게 해결
* 미리 학습된 인물 간의 비교를 통해 외부인의 정보 및 신원 빠르게 인식 가능
* 카카오 플랫폼을 도입하여 사용자의 접근성 및 편리함 증가
* 라즈베리파이 기기 자체에서 인터넷을 연결함으로써 알고리즘 간편화 및 속도 증가 
* 라즈베리파이 모니터를 인터폰으로 활용함으로써 효율성 증가
* 외출 시에도 손쉽게 문 앞 상황을 관리 및 감독 가능
* 접근하기 어려운 범죄 관련 증거물을 개인 소셜 미디어를 통해 쉽게 획득 
_______________
## :star: 활용 분야
...
- 문 앞 영상 스트리밍 기능을 활용해 의심 상황 실시간 확인
