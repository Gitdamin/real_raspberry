...
__________
## :star: Setting 
```raspberry pi 3B+, pi camera, python 3.7.3, subprocess, selenium, kakao```
...
에 접속하여 개인 카카오 채널 생성
5. 사용자 맞춤 설정<br/>
```tools/main```, ```detection/face(motion)_main``` code에서 ```kakao1, 2()```부분을 "개인 아이디, 비밀번호, 채팅방 주소"로 수정
```python
 def kakao1, 2():
...
```
6. ```detection/face_main```, ```tools/main```의 names=[ ] 수정
<br/>
